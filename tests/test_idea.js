...
// ── Enterprise resource subcommands (state-resource/priority-resource/property-resource/property-plan) ──

const ENTERPRISE_RESOURCE_SUBCOMMANDS = [
  'state-resource-list', 'state-resource-get',
  'priority-resource-list', 'priority-resource-get',
  'property-resource-list', 'property-resource-create',
  'property-resource-get', 'property-resource-update',
...
async function runIdeaQuietly(tokens) {
  let output = '';
  const originalLog = console.log;
  console.log = (...args) => { output += args.join(' ') + '\n'; };
  try {
    await idea.run(tokens);
  } finally {
    console.log = originalLog;
  }
  return output;
...
testInCleanEnv('idea --help shows enterprise resource subcommands and distinction', async () => {
  const output = await runIdeaQuietly(['--help']);
  for (const sub of ENTERPRISE_RESOURCE_SUBCOMMANDS) {
    assert.ok(output.includes(sub), `missing ${sub} in module help`);
  }
  assert.ok(output.includes('Enterprise configuration resources'));
});

testInCleanEnv('idea enterprise resource subcommands --help show usage', async () => {
  for (const sub of ENTERPRISE_RESOURCE_SUBCOMMANDS) {
    const output = await runIdeaQuietly([sub, '--help']);
    assert.ok(output.includes(`Usage: pingcode idea ${sub}`), `missing usage for ${sub}`);
  }
});

testInCleanTmp('idea state-resource-list dry-run returns flat GET structure', async (t, tmpdir) => {
  const cachePath = tmpFile(tmpdir, 'workspace.json');
  writeWorkspaceCache(cachePath, { preferences: {} });

  const output = await runIdeaQuietly([
    'state-resource-list', '--workspace-cache', cachePath, '--dry-run',
  ]);

  const result = JSON.parse(output.trim());
  assert.strictEqual(result.dry_run, true);
  assert.strictEqual(result.method, 'GET');
  assert.strictEqual(result.path, '/v1/ship/idea_states');
});

testInCleanTmp('idea state-resource-get dry-run interpolates state id', async (t, tmpdir) => {
  const cachePath = tmpFile(tmpdir, 'workspace.json');
  writeWorkspaceCache(cachePath, { preferences: {} });

  const output = await runIdeaQuietly([
    'state-resource-get', '63e1bf51898a0be5a2d21b2a', '--workspace-cache', cachePath, '--dry-run',
  ]);

  const result = JSON.parse(output.trim());
  assert.strictEqual(result.dry_run, true);
  assert.strictEqual(result.method, 'GET');
  assert.strictEqual(result.path, '/v1/ship/idea_states/63e1bf51898a0be5a2d21b2a');
});

testInCleanTmp('idea priority-resource-list dry-run returns flat GET structure', async (t, tmpdir) => {
  const cachePath = tmpFile(tmpdir, 'workspace.json');
  writeWorkspaceCache(cachePath, { preferences: {} });

  const output = await runIdeaQuietly([
    'priority-resource-list', '--workspace-cache', cachePath, '--dry-run',
  ]);

  const result = JSON.parse(output.trim());
  assert.strictEqual(result.dry_run, true);
  assert.strictEqual(result.method, 'GET');
  assert.strictEqual(result.path, '/v1/ship/idea_priorities');
});

testInCleanTmp('idea priority-resource-get dry-run interpolates priority id', async (t, tmpdir) => {
  const cachePath = tmpFile(tmpdir, 'workspace.json');
  writeWorkspaceCache(cachePath, { preferences: {} });

  const output = await runIdeaQuietly([
    'priority-resource-get', '5cb9466afda1ce4ca0090005', '--workspace-cache', cachePath, '--dry-run',
  ]);

  const result = JSON.parse(output.trim());
  assert.strictEqual(result.dry_run, true);
  assert.strictEqual(result.method, 'GET');
  assert.strictEqual(result.path, '/v1/ship/idea_priorities/5cb9466afda1ce4ca0090005');
});

testInCleanTmp('idea property-resource-list dry-run returns flat GET structure', async (t, tmpdir) => {
  const cachePath = tmpFile(tmpdir, 'workspace.json');
  writeWorkspaceCache(cachePath, { preferences: {} });

  const output = await runIdeaQuietly([
    'property-resource-list', '--workspace-cache', cachePath, '--dry-run',
  ]);

  const result = JSON.parse(output.trim());
  assert.strictEqual(result.dry_run, true);
  assert.strictEqual(result.method, 'GET');
  assert.strictEqual(result.path, '/v1/ship/idea_properties');
});

testInCleanTmp('idea property-resource-create dry-run returns POST with body', async (t, tmpdir) => {
  const cachePath = tmpFile(tmpdir, 'workspace.json');
  writeWorkspaceCache(cachePath, { preferences: {} });

  const output = await runIdeaQuietly([
    'property-resource-create',
    '--name', '严重程度',
    '--type', 'select',
    '--options', JSON.stringify([{ text: '严重' }, { text: '一般' }]),
    '--workspace-cache', cachePath, '--dry-run',
  ]);
  ...
  assert.strictEqual(result.method, 'POST');
  assert.strictEqual(result.path, '/v1/ship/idea_properties');
  assert.strictEqual(result.json.name, '严重程度');
  assert.strictEqual(result.json.type, 'select');
  assert.deepStrictEqual(result.json.options, [{ text: '严重' }, { text: '一般' }]);
});

testInCleanTmp('idea property-resource-create dry-run without options omits them', async (t, tmpdir) => {
  const cachePath = tmpFile(tmpdir, 'workspace.json');
  writeWorkspaceCache(cachePath, { preferences: {} });

  const output = await runIdeaQuietly([
    'property-resource-create', '--name', '备注', '--type', 'textarea',
    '--workspace-cache', cachePath, '--dry-run',
  ]);

  const result = JSON.parse(output.trim());
  assert.strictEqual(result.method, 'POST');
  assert.strictEqual(result.json.type, 'textarea');
  assert.strictEqual('options' in result.json, false);
});

testInCleanTmp('idea property-resource-get dry-run accepts slug ids', async (t, tmpdir) => {
  const cachePath = tmpFile(tmpdir, 'workspace.json');
  writeWorkspaceCache(cachePath, { preferences: {} });

  const output = await runIdeaQuietly([
    'property-resource-get', 'severity', '--workspace-cache', cachePath, '--dry-run',
  ]);

  const result = JSON.parse(output.trim());
  assert.strictEqual(result.dry_run, true);
  assert.strictEqual(result.method, 'GET');
  assert.strictEqual(result.path, '/v1/ship/idea_properties/severity');
});

testInCleanTmp('idea property-resource-update dry-run returns PATCH with partial body', async (t, tmpdir) => {
  const cachePath = tmpFile(tmpdir, 'workspace.json');
  writeWorkspaceCache(cachePath, { preferences: {} });

  const output = await runIdeaQuietly([
    'property-resource-update', 'severity', '--name', '严重程度-update',
    '--workspace-cache', cachePath, '--dry-run',
  ]);
  ...
  assert.strictEqual(result.method, 'PATCH');
  assert.strictEqual(result.path, '/v1/ship/idea_properties/severity');
  assert.strictEqual(result.json.name, '严重程度-update');
  assert.strictEqual('options' in result.json, false);
});

testInCleanTmp('idea property-resource-update dry-run with options array', async (t, tmpdir) => {
  const cachePath = tmpFile(tmpdir, 'workspace.json');
  writeWorkspaceCache(cachePath, { preferences: {} });

  const output = await runIdeaQuietly([
    'property-resource-update', 'severity',
    '--options', JSON.stringify([{ _id: '5efb1859110533727a82c603', text: '严重-update' }, { text: '一般' }]),
    '--workspace-cache', cachePath, '--dry-run',
  ]);

  const result = JSON.parse(output.trim());
  assert.strictEqual(result.method, 'PATCH');
  assert.deepStrictEqual(result.json.options, [
    { _id: '5efb1859110533727a82c603', text: '严重-update' },
    { text: '一般' },
  ]);
});

testInCleanTmp('idea property-plan-list dry-run returns flat GET structure', async (t, tmpdir) => {
  const cachePath = tmpFile(tmpdir, 'workspace.json');
  writeWorkspaceCache(cachePath, { preferences: {} });

  const output = await runIdeaQuietly([
    'property-plan-list', '--workspace-cache', cachePath, '--dry-run',
  ]);

  const result = JSON.parse(output.trim());
  assert.strictEqual(result.dry_run, true);
  assert.strictEqual(result.method, 'GET');
  assert.strictEqual(result.path, '/v1/ship/idea_property_plans');
});

testInCleanTmp('idea property-plan-get dry-run interpolates plan id', async (t, tmpdir) => {
  const cachePath = tmpFile(tmpdir, 'workspace.json');
  writeWorkspaceCache(cachePath, { preferences: {} });

  const output = await runIdeaQuietly([
    'property-plan-get', '5f8a21f18ef715265de90c22', '--workspace-cache', cachePath, '--dry-run',
  ]);

  const result = JSON.parse(output.trim());
  assert.strictEqual(result.dry_run, true);
  assert.strictEqual(result.method, 'GET');
  assert.strictEqual(result.path, '/v1/ship/idea_property_plans/5f8a21f18ef715265de90c22');
});

testInCleanTmp('idea property-plan-property-add dry-run returns nested POST', async (t, tmpdir) => {
  const cachePath = tmpFile(tmpdir, 'workspace.json');
  writeWorkspaceCache(cachePath, { preferences: {} });

  const output = await runIdeaQuietly([
    'property-plan-property-add', '5d7a21f19ef715269ae90c50', 'solution',
    '--workspace-cache', cachePath, '--dry-run',
  ]);
  ...
  assert.strictEqual(result.dry_run, true);
  assert.strictEqual(result.method, 'POST');
  assert.strictEqual(result.path, '/v1/ship/idea_property_plans/5d7a21f19ef715269ae90c50/idea_properties');
  assert.deepStrictEqual(result.json, { property_id: 'solution' });
});

testInCleanTmp('idea property-plan-property-get dry-run returns nested GET', async (t, tmpdir) => {
  const cachePath = tmpFile(tmpdir, 'workspace.json');
  writeWorkspaceCache(cachePath, { preferences: {} });

  const output = await runIdeaQuietly([
    'property-plan-property-get', '5d7a21f19ef715269ae90c50', 'solution',
    '--workspace-cache', cachePath, '--dry-run',
  ]);
  ...
  assert.strictEqual(result.dry_run, true);
  assert.strictEqual(result.method, 'GET');
  assert.strictEqual(result.path, '/v1/ship/idea_property_plans/5d7a21f19ef715269ae90c50/idea_properties/solution');
});

testInCleanTmp('idea property-plan-property-list dry-run returns nested GET', async (t, tmpdir) => {
  const cachePath = tmpFile(tmpdir, 'workspace.json');
  writeWorkspaceCache(cachePath, { preferences: {} });

  const output = await runIdeaQuietly([
    'property-plan-property-list', '5d7a21f19ef715269ae90c50', '--workspace-cache', cachePath, '--dry-run',
  ]);
...
testInCleanTmp('idea property-plan-property-remove dry-run returns nested DELETE', async (t, tmpdir) => {
  const cachePath = tmpFile(tmpdir, 'workspace.json');
  writeWorkspaceCache(cachePath, { preferences: {} });

  const output = await runIdeaQuietly([
    'property-plan-property-remove', '5d7a21f19ef715269ae90c50', 'solution',
    '--workspace-cache', cachePath, '--dry-run',
  ]);
...
// ── Enterprise resource error paths ───────────────────────────────────

testInCleanTmp('idea state-resource-get missing id errors', async (t, tmpdir) => {
  const cachePath = tmpFile(tmpdir, 'workspace.json');
  writeWorkspaceCache(cachePath, { preferences: {} });

  try {
    await runIdeaQuietly(['state-resource-get', '--workspace-cache', cachePath, '--dry-run']);
    assert.fail('Expected error was not thrown');
  } catch (exc) {
    assert.ok(exc.message.includes('idea state id'));
    assert.ok(exc.message.includes('required'));
    process.exitCode = 0;
  }
});

testInCleanTmp('idea property-resource-create missing --name errors', async (t, tmpdir) => {
  const cachePath = tmpFile(tmpdir, 'workspace.json');
  writeWorkspaceCache(cachePath, { preferences: {} });

  try {
    await runIdeaQuietly([
      'property-resource-create', '--type', 'select',
      '--workspace-cache', cachePath, '--dry-run',
    ]);
    assert.fail('Expected error was not thrown');
  } catch (exc) {
    assert.ok(exc.message.includes('--name'));
    assert.ok(exc.message.includes('required'));
    process.exitCode = 0;
  }
});

testInCleanTmp('idea property-resource-create missing --type errors', async (t, tmpdir) => {
  const cachePath = tmpFile(tmpdir, 'workspace.json');
  writeWorkspaceCache(cachePath, { preferences: {} });

  try {
    await runIdeaQuietly([
      'property-resource-create', '--name', '严重程度',
      '--workspace-cache', cachePath, '--dry-run',
    ]);
    assert.fail('Expected error was not thrown');
  } catch (exc) {
    assert.ok(exc.message.includes('--type'));
    assert.ok(exc.message.includes('required'));
    process.exitCode = 0;
  }
});

testInCleanTmp('idea property-resource-create invalid --type errors', async (t, tmpdir) => {
  const cachePath = tmpFile(tmpdir, 'workspace.json');
  writeWorkspaceCache(cachePath, { preferences: {} });

  try {
    await runIdeaQuietly([
      'property-resource-create', '--name', '严重程度', '--type', 'bogus',
      '--workspace-cache', cachePath, '--dry-run',
    ]);
    assert.fail('Expected error was not thrown');
  } catch (exc) {
    assert.ok(exc.message.includes('--type must be one of'));
    process.exitCode = 0;
  }
});

testInCleanTmp('idea property-resource-create --options invalid JSON errors', async (t, tmpdir) => {
  const cachePath = tmpFile(tmpdir, 'workspace.json');
  writeWorkspaceCache(cachePath, { preferences: {} });
  ...
  try {
    ...
    assert.ok(exc.message.includes('must be valid JSON'));
    process.exitCode = 0;
  }
});

testInCleanTmp('idea property-resource-create --options non-array rejects', async (t, tmpdir) => {
  const cachePath = tmpFile(tmpdir, 'workspace.json');
  writeWorkspaceCache(cachePath, { preferences: {} });
  ...
  try {
    ...
    assert.ok(exc.message.includes('must be a JSON array'));
    process.exitCode = 0;
  }
});

testInCleanTmp('idea property-resource-create --options entry without text rejects', async (t, tmpdir) => {
  const cachePath = tmpFile(tmpdir, 'workspace.json');
  writeWorkspaceCache(cachePath, { preferences: {} });
  ...
  try {
    ...
    assert.ok(exc.message.includes('text field'));
    process.exitCode = 0;
  }
});

testInCleanTmp('idea property-resource-update missing target errors', async (t, tmpdir) => {
  const cachePath = tmpFile(tmpdir, 'workspace.json');
  writeWorkspaceCache(cachePath, { preferences: {} });

  try {
    await runIdeaQuietly([
      'property-resource-update', '--name', 'New name',
      '--workspace-cache', cachePath, '--dry-run',
    ]);
    assert.fail('Expected error was not thrown');
  } catch (exc) {
    assert.ok(exc.message.includes('idea property id'));
    assert.ok(exc.message.includes('required'));
    process.exitCode = 0;
  }
});

testInCleanTmp('idea property-resource-update with target but no fields errors', async (t, tmpdir) => {
  const cachePath = tmpFile(tmpdir, 'workspace.json');
  writeWorkspaceCache(cachePath, { preferences: {} });
  ...
  try {
    ...
    assert.ok(exc.message.includes('At least one field to update is required'));
    process.exitCode = 0;
  }
});

testInCleanTmp('idea property-plan-get missing id errors', async (t, tmpdir) => {
  const cachePath = tmpFile(tmpdir, 'workspace.json');
  writeWorkspaceCache(cachePath, { preferences: {} });

  try {
    await runIdeaQuietly(['property-plan-get', '--workspace-cache', cachePath, '--dry-run']);
    assert.fail('Expected error was not thrown');
  } catch (exc) {
    assert.ok(exc.message.includes('property plan id'));
    assert.ok(exc.message.includes('required'));
    process.exitCode = 0;
  }
});

testInCleanTmp('idea property-plan-property-add missing property id errors', async (t, tmpdir) => {
  const cachePath = tmpFile(tmpdir, 'workspace.json');
  writeWorkspaceCache(cachePath, { preferences: {} });

  try {
    await runIdeaQuietly([
      'property-plan-property-add', '5d7a21f19ef715269ae90c50',
      '--workspace-cache', cachePath, '--dry-run',
    ]);
    assert.fail('Expected error was not thrown');
  } catch (exc) {
...
testInCleanTmp('idea property-plan-property-remove missing plan id errors', async (t, tmpdir) => {
  const cachePath = tmpFile(tmpdir, 'workspace.json');
  writeWorkspaceCache(cachePath, { preferences: {} });

  try {
    await runIdeaQuietly([
      'property-plan-property-remove',
      '--workspace-cache', cachePath, '--dry-run',
    ]);
    assert.fail('Expected error was not thrown');
  } catch (exc) {
...
testInCleanTmp('idea state-resource-list with unexpected positional errors', async (t, tmpdir) => {
  const cachePath = tmpFile(tmpdir, 'workspace.json');
  writeWorkspaceCache(cachePath, { preferences: {} });

  try {
    await runIdeaQuietly([
      'state-resource-list', 'extra', '--workspace-cache', cachePath, '--dry-run',
    ]);
    assert.fail('Expected error was not thrown');
  } catch (exc) {
...
testInCleanTmp('idea property-resource-create unknown option errors', async (t, tmpdir) => {
  const cachePath = tmpFile(tmpdir, 'workspace.json');
  writeWorkspaceCache(cachePath, { preferences: {} });

  try {
    await runIdeaQuietly([
      'property-resource-create', '--name', '严重程度', '--type', 'select', '--bogus',
      '--workspace-cache', cachePath, '--dry-run',
    ]);
    assert.fail('Expected error was not thrown');
  } catch (exc) {
    assert.ok(exc.message.includes('Unknown option: --bogus'));
    process.exitCode = 0;
  }
});
