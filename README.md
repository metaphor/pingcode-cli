...
一条命令会检测当前用户已存在的 Codex / OpenCode 目录，并只安装到这些已有 Agent。每个 Agent 的 skills 根目录下会安装三个同级 skill 目录：`pingcode-auth`、`pingcode-ctx`、`pingcode-workitem`：
...
2. 安装结束后请检查对应 Agent skills 目录下是否存在 `pingcode-auth`、`pingcode-ctx`、`pingcode-workitem` 三个同级目录，且每个目录里都有 SKILL.md 入口文件（按你当前使用的 Agent 选择对应路径即可）：
   - ~/.codex/skills/{pingcode-auth,pingcode-ctx,pingcode-workitem}/SKILL.md
   - ~/.config/opencode/skills/{pingcode-auth,pingcode-ctx,pingcode-workitem}/SKILL.md
...
以上能力按三个 skill 拆分提供：`$pingcode-auth`（用户令牌登录与令牌类型）、`$pingcode-ctx`（工作区上下文初始化）、`$pingcode-workitem`（工作项 list/create/show/get/update，含共享安全规则与 references 参考文档）。

## Skill 结构

安装后在同一个 skills 根目录下会有三个同级 skill：

| Skill | 作用 |
|---|---|
| `pingcode-auth` | 用户令牌登录：`pingcode auth login`、grant-type 自动识别与覆盖、令牌类型说明 |
| `pingcode-ctx` | 工作区上下文初始化：在 Agent 前台按编号选择当前项目、迭代、用户并写入缓存 |
| `pingcode-workitem` | 工作项操作：`workitem list / create / show / get / update` 子命令及完整参数；捆绑 `references/` 下的 API 摘要与操作流程文档 |
...
大模型应根据语义自动选择 `$pingcode-workitem` skill，并把自然语言转换成通用 CLI 命令和参数。不要为每个自然语言场景新增专门命令；统一使用 `scripts/pingcode.js` 的通用命令组合完成。

如果平台没有稳定的隐式 skill 选择能力，可以显式写：

```text
使用 $pingcode-workitem 查看我当前没完成的任务
...
在 Codex、OpenCode 等 Agent 产品里，推荐显式调用 `$pingcode-ctx`：
...
使用 `$pingcode-workitem` skill 执行常规工作项查询或创建前，应先确认工作区缓存里有 `current_user_id`、`current_project_id`、`current_sprint_id`。缺少任一项时先运行 `pingcode context init`，完成后再重试原来的 PingCode 操作。

查询工作项时，CLI 会自动补当前用户、当前项目、当前迭代过滤条件。用户明确要求“所有人”“全部项目”“全部迭代”时分别加 `--all-users`、`--all-projects`、`--all-sprints`。

## 参考资料

- 工作区上下文：[skills/pingcode-ctx/SKILL.md](skills/pingcode-ctx/SKILL.md)
