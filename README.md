...
一条命令会检测当前用户已存在的 Codex / OpenCode 目录，并只安装到这些已有 Agent。每个 Agent 的 skills 根目录下会安装四个同级 skill 目录：`pingcode`、`pingcode-auth`、`pingcode-ctx`、`pingcode-workitem`：
...
安装到自定义目录（例如项目本地的 `.codex/skills/pingcode` 或 OpenCode 项目级 `.opencode/skills/pingcode`）：

```bash
npx pingcode-cli@latest --target ".codex/skills/pingcode" --force
...
在 Codex、OpenCode 等 Agent 产品里，推荐显式调用 `$pingcode context init`：
